 ...
 * 		assumes these files are present, and if any new ones were to be added, they must be declared within the CardDeck
 * 		class. 
...
public class Main {
	...
	private static Combo playerSelect;
	private static Combo speedSelect;
	private static Combo modeSelect;
	private static Combo deckSelect;
	...
	public static void startGame() {
		...
		int playerCount = playerSelect.getSelectionIndex() + 1;
		int gameMode = modeSelect.getSelectionIndex();
		int deckNumber = deckSelect.getSelectionIndex();
		int memSpeed = MAXDELAY / (speedSelect.getSelectionIndex() + 1); // divides the maximum delay(ms) by speed.
		// Initialize game classes using user settings.
		currentDeck = new CardDeck(deckNumber);
	...
	private static void mainMenuUI() {
		...
		playerSelect = new Combo(playerSelectRow, SWT.DROP_DOWN | SWT.READ_ONLY);
		...
		speedSelect = new Combo(speedSelectRow, SWT.DROP_DOWN | SWT.READ_ONLY);
		...
		modeSelect = new Combo(modeSelectRow, SWT.DROP_DOWN | SWT.READ_ONLY);
		...
		deckSelect = new Combo(deckSelectRow, SWT.DROP_DOWN | SWT.READ_ONLY);
	...
	private static void enableGameCanvas() {
		...
		if (gameShell == null || gameShell.isDisposed()) {
			...
			int cardSize = currentDeck.getDeckList().get(0).getFront().getImageData().width;
			int gameX = cardSize * 4;
			int gameY = cardSize * (currentDeck.getDeckList().size() / 2);
	...
	static class CanvasPaintListener implements PaintListener {
		...
		public void paintControl(PaintEvent event) {
			 ...
			 */
			List<Card> gameBoard = currentBoard.getBoard();
			...
			ImageData data = currentDeck.getDeckList().get(0).getCurrentFace().getImageData();
	...
	static class CanvasMouseListener implements MouseListener {
		...
		public void mouseDown(MouseEvent event) {
			...
			Rectangle rect = canvas.getClientArea();
			ImageData data = currentDeck.getDeckList().get(0).getFront().getImageData();
